...
class Parser 
{
  ...
  parseClassDeclaration() 
  {
    ...
    parseAssignment() 
    {
      ...
      if (!op || op.value !== "=") 
      {
        error = this.addError(`Invalid assignment syntax: Expected '=' after identifier '${identifier?.value}'instead of '${op?.value}'`, ErrorType.SYNTAX, op?.loc || identifier?.loc);
      } 
      else 
      {
      ...
      if (keyword && !this.declareVariable(identifier.value, keyword.value)) 
      {
        ...
        {
          error = this.addError(`Variable '${identifier.value}' already declared`, ErrorType.SEMANTIC, this.tokens[0]?.loc);
        }
      }
    ...
    parseExpression() 
    {
      ...
      while (this.tokens.length > 0 && this.tokens[0].type === "operator") 
      {
        ...
        if (op.value === "+") 
        {
          ...
          {
            left = {type: "BinaryExpression", left: left, operator: op.value, right: right, dataType: "String",};
            continue;
          }
        }
        if (left.dataType === "String" || right.dataType === "String") 
    ...
    parseSimpleExpression() 
    {
      ...
      return {type: "BinaryExpression", left: left, operator: op.value, right: right, dataType: left.dataType};
    }
