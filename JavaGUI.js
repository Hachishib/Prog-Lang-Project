...
document.addEventListener("DOMContentLoaded", function () {
  const inputWrite = document.getElementById("inputWrite"); // Variable that Connects to html For Display
  ...
  // Function to set the current programming language based on button click
  ...
  languageButtons.forEach((btn) => {
    // Click then The language is change
  ...
  convertButton.addEventListener("click", () => {
    // It performs lexical analysis and parsing based on selected language
    ...
    switch (currentLanguage) {
      case "java":
        ...
        break;
      case "c":
        results = window.analyzeAndParseC(input); // From ProjGuiforC.js
        window.displayOutput(results.lexOutput); // Debugging
        window.displayAST(results.ast);
    ...
    displayOutput(results.lexOutput); //Display the Result Regardless of the Lanuguage selecterd
    displayAST(results.ast);
    displayErrors(results.errors);
  });

  function displayOutput(output) {
    // Function to display lexical output
    outputWrite.value = output;
  }
  // Function to display the AST (Abstract Syntax Tree)
  function displayAST(ast) {
    astOutput.innerHTML = "";
    const cleanAST = ast.filter((node) => !node.error); // Filter out AST nodes with errors
  ...
  // Function to display errors (syntax and semantic)
  function displayErrors(errors) {
    ...
    function addErrorItems(errorList) {
      // Helper function to append error messages to the output
  ...
  class Parser {
    ...
    synchronize() {
      ...
      while (this.tokens.length > 0) {
        ...
        if (token.value === "{") {
          ...
          //Statement end for ; or Start of new statement
        ...
        if (op.value === "+") {
          if (left.dataType === "String" || right.dataType === "String") {
            left = {
              ...
              right: right,
              dataType: "String",
            }; // Result type is String for concatenation
            continue;
          }
        }
    ...
    parseLiteralOrExpression(token) {
      ...
      if (token.type === "keyword" && typeKeywords.includes(token.value)) {
        ...
        this.addError(
          "Invalid declaration in expression",
          ErrorType.SEMANTIC,
          token.loc
        );
        this.synchronize();
    ...
    parseIfStatement(context = "if") {
      ...
      while (this.tokens[0]?.value === "else") {
        ...
        if (this.isIfKeyword(this.tokens[0])) {
          ...
          if (!elseIfCondition) {
            this.addError(
              "Invalid condition in 'else if' statement",
              ErrorType.SYNTAX
            );
            this.synchronize();
            continue;
    ...
    parseCondition() {
      ...
      if (
        ...
        this.addError(
          `Invalid condition data type '${expression.dataType}'`,
          ErrorType.SEMANTIC
        );
      }
      ...
      if (loopType === "do") {
        ...
        if (this.isAssignment(this.tokens)) {
          ...
          if (
            ...
            if (
              !this.consumeToken(
                ";",
                "Expected ';' after initialization and before condition in for loop"
              )
            ) {
              return null;
            }
        ...
        if (this.tokens[0]?.value === ";") {
          ...
          if (
            !this.consumeToken(
              ";",
              "Expected ';' after condition and before iteration in for loop"
            )
          ) {
            return null;
          }
